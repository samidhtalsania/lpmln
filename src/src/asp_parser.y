...
//ex 0.8536 !(B => Bottom)
//Test case covered
rule(R) ::= number REVERSE_IMPLICATION NEGATION LBRACKET ruleU(R1) RBRACKET.{
	R = R1;
	R->toBeCompleted = false;
}

rule(R) ::= number REVERSE_IMPLICATION LBRACKET ruleU(R1) RBRACKET.{
	R = R1;
	R->toBeCompleted = false;
}
...
//ex !(B => Bottom).
//Test case wrong
rule(R) ::= REVERSE_IMPLICATION NEGATION LBRACKET ruleU(R1) RBRACKET DOT.{
	R = R1;
...
//Top => H.
//Test case covered
rule(R) ::= number body(B) DISJUNCTION bodydef(B1).{
	//Doing this 
	R = new RuleCompletion;
	R->isHeadTop = true;
	RULE_COMPLETION_BODY_TOP(B,B1)
...
//0.8536 B => H
//Test case covered
rule(R) ::= number head(H) REVERSE_IMPLICATION body(B). {
	RULE_COMPLETION_BH(B,H);
	R = new RuleCompletion(H->getPredicate(),predList, resultMap, varMap);
	delete B;
	delete H;
